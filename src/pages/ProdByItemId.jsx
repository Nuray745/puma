...
function ProdByItemId() {
  ...
  useEffect(() => {
    getProductsByItemId(itemId).then((data) => {
      setAllProducts(data || []);
      setProducts(data || []);
    });
  }, [itemId]);
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2 tablet:gap-3 text-xs tablet:text-base text-[#191919]">
            ...
            <Link
              to={`/categories/${
                categories.find((cat) =>
                  cat.subcategory.some((sub) => sub.id == categoryId)
                )?.id
              }`}
            ...
            <div className="w-1 h-1 mt-1 rounded-full bg-[#8C9198]"></div>
            {
              categories
                .flatMap((cat) =>
                  cat.subcategory.flatMap((sub) => sub.children || [])
                )
                .find((child) => child.id == itemId)?.categoryName
            }
          </div>
